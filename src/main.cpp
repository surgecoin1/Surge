...
unsigned int nStakeTargetSpacing = 1 * 60; // 
unsigned int nStakeMinAge = 1 * 24 * 60 * 60; //
unsigned int nStakeMaxAge = 99 * 24 * 60 * 60; //
...
const string strMessageMagic = "Surge Coin Signed Message:\n";
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Surge Coin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
